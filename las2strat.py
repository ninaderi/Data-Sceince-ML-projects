...
"This is the main program to convert LAS files to Strat files in batch, automatically. " \
"Note: This program works well on a local drive, but it won't work on the pCloud." \
" Therefore, it is advised to copy and paste your files to a local drive before running this program"
...
def create():
    ...
    lst = os.listdir(input_0 + "\\")

    name_list_input = []


    for index in range(len(lst)):
        name_list_input.append(input_0 + "/" + lst[index])
        # print(name_list_input[index])
    # print(name_list_input)

    for index in range(len(lst)):
        f = open(name_list_input[index], 'r')
        f_contents = f.readlines()
        las_to_strat = name_list_input[index].replace('LAS', 'strat')
